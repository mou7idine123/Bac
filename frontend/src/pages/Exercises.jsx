...
import { Clock, CheckSquare, Award, Play, X, ChevronRight, FileSearch, BookOpen, Search, AlertCircle, CheckCircle, Circle } from 'lucide-react';
...
export default function Exercises() {
  ...
  const handleOpenExercise = (ex) => {
    if (ex.pdf_path) {
      ...
      alert("Cet exercice n'a pas de PDF attaché.");
    }
  };
          ...
          {exercises.length === 0 ? (
            ...
            <div className="grid stagger anim-fade-up" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.25rem', marginBottom: '2rem' }}>
              {exercises.filter(ex => {
                ...
                const cardColor = ex.type === 'Interactif' ? '#8b5cf6' : '#10b981';
                    ...
                    <div style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                        <div style={{
                          ...
                          {ex.type === 'Interactif' ? <Play size={20} /> : <BookOpen size={20} />}
                        </div>
                        <span style={{
